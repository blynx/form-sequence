...
export default class FormSequence extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.setStep(0)
    this._homeUrl = Object.assign({}, window.location)
  ...
  handleNextStep(event, nextThing, options) {
    ...
    if (nextThing instanceof HTMLFormElement) {
      ...
      req.url = new URL(nextThing.getAttribute("action"), window.location)
      ...
      req.url = new URL(nextThing, window.location)
    }

    this.dispatchStartEvent({ request: req })
    fetch(req.url, {
      method: req.method,
      body: req.body,
      headers: req.headers,
      ...
      .then(response => {
        Promise
          ...
          .then(([ok, responseUrl, nextDocumentString, response]) => {
            ...
            if (ok || config.passingStatusCodes.includes(response.status)) {
  ...
  insertForm(form) {
    ...
    window.requestAnimationFrame(() => {
      ...
      let textInputLike = "input" +
        ["button", "checkbox", "color", "file", "hidden", "image", "radio", "range", "reset", "submit"]
          .map(t => `:not([type="${t}"])`)
          .join("")
      let nextTextInput = this._remoteContainer.querySelector(textInputLike)
  ...
  handleCancel(event) {
    ...
    this.dispatchEvent(new CustomEvent("cancel", {detail: {...detail}}))
    this.close()
  }

  dispatchStartEvent(detail) {
  ...
  dispatchReturnEvent(detail) {
    ...
    this.dispatchEvent(new CustomEvent("return", {detail: {...detail}}))
  }
