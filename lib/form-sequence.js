...
export default class FormSequence extends HTMLElement {
  ...
  handleNextStep(event, url) {
    ...
    fetch(url, { headers: config.requestHeaders })
      .then(response => {
        Promise
          .all([response.ok, response.text(), response])
          .then(([ok, nextDocumentString, response]) => {
  ...
  handleSubmit({event, node: form}) {
    ...
    fetch(action, {method, body, headers: config.requestHeaders})
